...
\url{https://shahriar88.github.io/}\\[0.6em]
\qrcode[height=1.2in]{https://shahriar88.github.io}
}
\date{\today}
...
\multicolumn{2}{l}{\textbf{Basic Linux Commands}} \\

\texttt{pwd} & Print current working directory. \\
\texttt{ls} & List files in current directory. \\
\texttt{ls -l} & List files with detailed information. \\
\texttt{cd <dir>} & Change directory. \\
\texttt{mkdir <dir>} & Create a new directory. \\
\texttt{cp <src> <dst>} & Copy file or directory. \\
\texttt{mv <src> <dst>} & Move or rename file. \\
\texttt{rm <file>} & Remove file. \\

\multicolumn{2}{l}{\textbf{SLURM / HPC Commands}} \\

\texttt{sinfo} & Show available partitions and node status. \\
\texttt{squeue} & Show all running jobs. \\
\texttt{squeue -u <user>} & Show jobs for specific user. \\
\texttt{sbatch <script.sh>} & Submit batch job. \\
\texttt{srun --pty bash} & Start interactive session. \\
\texttt{scancel <jobid>} & Cancel a running job. \\

\multicolumn{2}{l}{\textbf{GPU and Monitoring}} \\

\texttt{nvidia-smi} & Display GPU usage (compute nodes only). \\
\texttt{tail -f <file>} & Monitor job output in real time. \\

\multicolumn{2}{l}{\textbf{Environment and Modules}} \\

\texttt{module avail} & List available software modules. \\
\texttt{module load <name>} & Load a software module. \\
\texttt{module list} & Show currently loaded modules. \\

\multicolumn{2}{l}{\textbf{Storage and Usage}} \\

\texttt{du -sh *} & Show directory sizes. \\
\texttt{df -h} & Show disk usage. \\
\texttt{quota -s} & Show storage quota. \\

\hline
\end{tabular}
\caption{Common Linux and SLURM commands for CRADLE HPC usage}
\end{table}
...
Performance & Slowest & Fast & Medium & Fastest \\
Scalability & Poor & Limited & Limited & Excellent \\
Communication & None & None & Gather/scatter overhead & NCCL all-reduce \\
Gradient sync & N/A & N/A & Implicit (less efficient) & Explicit (efficient) \\
Memory efficiency & High & Medium & Lower (replication overhead) & Medium (replicated per GPU) \\
Fault isolation & N/A & N/A & Poor & Better \\
Code complexity & Very low & Low & Low & Medium \\
Launch method & \texttt{python} & \texttt{python} & \texttt{python} & \texttt{torchrun}/\texttt{srun} \\
Best use case & Debugging, tests & Small models & Prototyping & Large-scale training \\
Recommended for HPC & No & Limited & No & Yes \\
...
\begin{itemize}[leftmargin=*]
  ...
  \item Expect slower scaling beyond 2–4 GPUs.
...
\begin{codeblock}
scp my_script.py \
    yourUserName@login.cradle.university.edu:/home/yourUserName/projects/
\end{codeblock}

This copies \texttt{my\_script.py} from your current local directory into
\texttt{/home/yourUserName/projects/} on the cluster.

\subsubsection{Copying a Single File from CRADLE to Local Machine}

\begin{codeblock}
scp yourUserName@login.cradle.university.edu:/home/yourUserName/projects/results.txt \
    .
\end{codeblock}
...
This creates \texttt{/home/yourUserName/projects/myProjectFolder/} on the cluster.

\subsubsection{Copying an Entire Directory from CRADLE to Local Machine}

\begin{codeblock}
scp -r yourUserName@login.cradle.university.edu:/home/yourUserName/projects/myProjectFolder/ \
    ~/Desktop/
\end{codeblock}
...
\begin{codeblock}
scp yourUserName@login.cradle.university.edu:/home/yourUserName/slurm_outputs/myJob.out.123456 \
    .
\end{codeblock}
...
\begin{codeblock}
scp yourUserName@login.cradle.university.edu:/home/yourUserName/slurm_outputs/*.out.* \
    .
\end{codeblock}
