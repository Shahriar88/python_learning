...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 25 10:48:55 2026

@author: kec994
"""

"""Convert PDF files to structured Markdown, separately or in batches. Code run and verified"""
...
Write modes:
    replace -> overwrite an existing destination file
    append  -> append to an existing destination file
    skip    -> leave an existing destination file unchanged
"""
...
def write_output(
    ...
    """
    Write content using replace, append, or skip mode.

    Returns True when content is written.
    Returns False when skip mode leaves an existing file unchanged.
    """
    ...
    if operation == "skip" and destination_file.exists():
        return False

    file_mode = "a" if operation == "append" else "w"
...
if len(sys.argv) not in {3, 4, 5, 6}:
    raise SystemExit(
        ...
        "Single PDF file:\n"
        "  py pdf_to_structured_text.py "
        "Source_PDF Destination_File [single] [replace|append|skip]\n"
        "\n"
        "Source folder with separate outputs:\n"
        "  py pdf_to_structured_text.py "
        "Source_Folder Destination_Folder "
        "[separate] [replace|append|skip]\n"
        "\n"
        "Source folder with one combined output:\n"
        "  py pdf_to_structured_text.py "
        "Source_Folder Destination_File "
        "combine [replace|append|skip] [PDFs_Per_File]\n"
        "\n"
        "Example: create one output file for every 5 PDFs:\n"
        "  py pdf_to_structured_text.py "
        "Source_Folder Destination_File combine replace 5"
...
if operation not in {"replace", "append", "skip"}:
    raise SystemExit(
        "Write mode must be 'replace', 'append', or 'skip'."
    )
...
if output_mode == "single":
    ...
    if operation == "skip" and destination.exists():
        print("Processing complete.")
        print(f"Source PDF file: {source}")
        print(f"Destination already exists. Skipped: {destination}")
        print(f"Write mode: {operation}")
...
# ============================================================
    ...
    if output_mode == "separate":
        ...
        for number, pdf_path in enumerate(
            ...
            if operation == "skip" and destination_file.exists():
                ...
                print(
                    f"Destination already exists. Skipped: "
                    f"{destination_file}"
                )
    ...
    # ========================================================
        ...
        for batch_number, pdf_batch in enumerate(
            ...
            # In skip mode, an existing combined destination is left
            # completely unchanged, so every PDF assigned to that batch
            # is counted as skipped.
            if operation == "skip" and batch_destination.exists():
                skipped += len(pdf_batch)

                print(
                    f"Destination already exists. "
                    f"Skipped entire batch: {batch_destination}"
                )

                continue

            file_mode = (
                "a" if operation == "append" else "w"
            )
    ...
    print(f"PDF files found: {len(pdf_files)}")
    print(f"Successfully processed: {successful}")
    print(f"Skipped existing files/PDFs: {skipped}")
    print(f"Failed: {failed}")
    print(f"Output mode: {output_mode}")
    print(f"Write mode: {operation}")
    if output_mode == "combine":
...
# Skip one source PDF if its destination Markdown file already exists:
# py .\pdf_to_structured_text.py ".\part1\paper1.pdf" ".\paper1.md" single skip

# Read every PDF recursively and create only missing Markdown files.
# Existing destination Markdown files are left completely unchanged:
# py .\pdf_to_structured_text.py ".\part1" ".\structured_papers" separate skip

# Combine all PDFs into one file only if the combined destination does not exist:
# py .\pdf_to_structured_text.py ".\part1" ".\all_papers.md" combine skip

# Create combined files in groups of 5, skipping any numbered batch file
# that already exists:
# py .\pdf_to_structured_text.py ".\part1" ".\all_papers.md" combine skip 5
